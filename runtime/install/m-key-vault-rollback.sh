#!/usr/bin/env bash
# m-key-vault-rollback.sh — one-command rollback for m-key-vault.sh.
#   sudo bash m-key-vault-rollback.sh <secret-suffix>
...
#   2. Deletes the OneCLI secret vitaliy-<secret-suffix>.

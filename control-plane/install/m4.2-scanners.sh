...
# Idempotent. Pilot: vitaliy only. The scanner package is a library + the
...
runuser -u vitaliy -- bash -lc "cd '$REPO' && corepack pnpm install" || die "pnpm install failed"
...
runuser -u vitaliy -- bash -lc "cd '$REPO' && node_modules/.bin/tsx packages/scanners/src/smoke.ts" \

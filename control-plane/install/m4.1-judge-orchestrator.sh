...
#
#   - ensures bullmq is installed (pnpm install, run as vitaliy — needs a writable
#     HOME cache, which exists on the host but NOT inside a tenant pod)
...
#
# Run as root on the host. Idempotent. Pilot: vitaliy only. Teardown-tracked.
...
# ---- 1) deps (bullmq) — install as vitaliy on the host -----------------------
log "pnpm install (adds bullmq for @fleet/judge-orchestrator)"
runuser -u vitaliy -- bash -lc "cd '$REPO' && corepack pnpm install" \
  || die "pnpm install failed — check vitaliy can reach the npm registry via the proxy"
...
runuser -u vitaliy -- bash -lc "cd '$REPO' && node_modules/.bin/tsx apps/judge-orchestrator/src/smoke.ts" \
...
JUDGE_URL="http://127.0.0.1:$ACCEPT_PORT" runuser -u vitaliy -- bash -lc "cd '$REPO' && node install/m4.1-accept.mjs" \
